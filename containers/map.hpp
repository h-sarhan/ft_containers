...
/*   Updated: 2023/03/03 16:36:56 by hsarhan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
template <class Key, class T, class Compare, class Alloc>
typename ft::map<Key, T, Compare, Alloc>::mapped_type			&ft::map<Key, T, Compare, Alloc>::operator[](const key_type &key)
{
	node_type *res = _bst.get(key);
	if (res == NULL)
	{
		// Should create an element with a value of mapped_type()
		throw std::out_of_range("key not in map"); // ! Replace this by returning a reference to the value that was just added
	}
	return res->data.second;
}

template <class Key, class T, class Compare, class Alloc>
typename ft::map<Key, T, Compare, Alloc>::allocator_type												ft::map<Key, T, Compare, Alloc>::get_allocator(void) const
{
	return _alloc;
}
