...
template <class MapTree> class const_map_iterator
{
  public:
    // * Types
    typedef ptrdiff_t difference_type;
    typedef const typename MapTree::node_data value_type;
    typedef value_type *pointer;
    typedef value_type &reference;
    typedef typename MapTree::node_type node_type;
    typedef node_type *node_pointer;
  ...
  public:
    const_map_iterator(void) : _ptr(NULL), _tree(NULL)
    {
    }

    const_map_iterator(node_pointer ptr, const tree_type *tree) : _ptr(ptr), _tree(tree)
    {
    }

    template <class T2>
    const_map_iterator(const map_iterator<T2> &old) : _ptr(old.base()), _tree(old.tree())
    {
    }

    template <class T2> const_map_iterator &operator=(const const_map_iterator<T2> &rhs)
    {
        if (this == &rhs)
        {
            return *this;
